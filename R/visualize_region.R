...
#' @title Visualize region of CARTA output
#' @description \code{visualize_region} Visualize region of CARTA output
#'
...
#' @export
#' @examples
#' # visualize_region.tmp <- visualize_region(seuratobj = gexatac_merge, df.genome = df.genome,  target = target, ranges.links.final = roi)
...
visualize_region <- function(seuratobj,
  ...
  ranges.links <- cbind(ranges.links, peaks.name)
    ...
    if(nrow(ranges.links.final) == 0){
      ...
      sub.chr.tmp <- rbind(sub.chr.tmp, data.frame(gr.ranges)[1:3] )
